...
def oulu(hard_protocol=None):
    oulu_path = "/home/jupyter/datasets/spoof/oulu/NO_ALIGNMENT/"
    protos = os.listdir(oulu_path)
    train_imgs = []
    val_imgs = []

    if hard_protocol is not None:
        train_path = f"{oulu_path}{hard_protocol}/Train"
        val_path = f"{oulu_path}{hard_protocol}/Dev"
        
        train_real, train_spoof = get_real_spoof_split(train_path)
        val_real, val_spoof = get_real_spoof_split(val_path)
        ...
        for proto in protos:
            train_imgs.extend(glob.glob(f"{oulu_path}/{proto}/Train/data/**/*"))
            val_imgs.extend(glob.glob(f"{oulu_path}/{proto}/Dev/data/**/*"))
...
def mobile(hardproto=None):
    train_path = "/home/jupyter/datasets/spoof_data/cropped_faces/mobile-replay/NO_ALIGNMENT/grandtest/train"
    val_path = "/home/jupyter/datasets/spoof_data/cropped_faces/mobile-replay/NO_ALIGNMENT/grandtest/devel"
...
def get_dataloaders(data_type="oulu", color_mode=['rgb'], batch_size=32, hard_protocol=None):
    transformations = get_transforms(color_mode)
    get_data = globals().get(data_type)
    im_data = get_data(hard_protocol)
